...
Route::get('/', function () {
	...
	$negocio = Negocio::all()->first()->url;

	return redirect()->route('url_negocio.index', compact('negocio'));
	// if (auth()->check()) {
	// 	return redirect()->route('redirigir');
	// }else{
	// 	return redirect()->route('acceder');
...
Route::post('/{url_negocio}/contacto', 'Negocio\NegocioController@formulario_contacto')->name('contacto');